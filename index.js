...
createMenuItem("sushi",25,"dinner");
...
// reviews.unshift({name:"Ruben",rating:4,feedback:"Great worker"})
...
*/
 ...
 function getReviewByRating(array,n) {
    let reviewRating = [];
    for(let i in array){
      if(array[i].rating >= 4.0 && array[i].rating <= 4.9){
        reviewRating.push(array[i])
      }
    }
    return reviewRating;
  }
...
function getLongReviews(array) {
    ...
    for(let i=0;i < array.length;i++){
      let wordLength = array[i].feedback.split(" ").length;
      if(wordLength > 15){
        long.push(array[i])
      }
    }
